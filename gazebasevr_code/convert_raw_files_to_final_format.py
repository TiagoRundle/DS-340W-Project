...
def convert_recording(df: pd.DataFrame) -> Optional[pd.DataFrame]:
...
def convert_subject_recordings(files: Sequence[Path], dest: Path) -> None:
    out_queue: List[Tuple[pd.DataFrame, Path]] = []
    for file in files:
        df = load_recording(file)
        out_df = convert_recording(df)
        if out_df is None:
...
def convert_all_recordings(src: Path, dest: Path) -> None:
    ...
    for subject_id, file_indices in id_to_file_indices.items():
        ...
        convert_subject_recordings(subject_files, dest)
